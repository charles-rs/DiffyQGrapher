...
public class Parser {
    ...
    /*
     * public static void main(String[] args) throws SyntaxError { Reader r = new
     * StringReader("dx/dt = 1 + 2 + a + 3 + 4\n"); Tokenizer t = new Tokenizer(r); Derivative dr =
     * parseDerivative(t); Node d = dr.differentiate('y'); System.out.println(dr.prettyPrint(new
     * StringBuilder())); System.out.println(dr.collapse().prettyPrint(new StringBuilder()));
     * //System.out.println(d.prettyPrint(new StringBuilder()));
     * //System.out.println(d.collapse().prettyPrint(new StringBuilder())); try {
     * System.out.println(d.eval(.8, .3, 2, 14, 4)); System.out.println(d.collapse().eval(.8, .3, 2,
     * 14, 4)); } catch (EvaluationException ex) { ex.printStackTrace(); } }
     */
    ...
    private static Node parseExpressionBack(Tokenizer t) throws SyntaxError {
        ...
        if (t.peek().getType() == TokenType.PLUS) {
            ...
            return temp;
        if (t.peek().getType() != TokenType.RPAREN && t.peek().getType() != TokenType.NL
                && t.peek().getType() != TokenType.EOF && t.peek().getType() != TokenType.DX
    ...
    private static Node parseTermBack(Tokenizer t) throws SyntaxError {
        ...
        if (t.peek().getType() != TokenType.RPAREN && t.peek().getType() != TokenType.MINUS
                && t.peek().getType() != TokenType.PLUS && t.peek().getType() != TokenType.NL
    ...
    private static Node parseDeriv(Tokenizer t) throws SyntaxError {
        ...
        if (temp.getType() == TokenType.DY)
            ...
            throw new SyntaxError("bad differential: " + temp.toString());
        consume(t, TokenType.DIV);
    ...
    private static Node parseValue(Tokenizer t) throws SyntaxError {
        ...
        switch (temp.getType()) {
            ...
            case R:
                return new Function(
                        Maths.add(new Function(new Value(ValueTypes.X), new Value(2D)),
    ...
    private static void consume(Tokenizer t, TokenType tt) throws SyntaxError {
        if (t.peek().getType() == tt)
            ...
            throw new SyntaxError("tried to consume the wrong type");
    }
