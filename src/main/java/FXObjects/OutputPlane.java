...
//    }
    ...
    public void renderSaddleCon(Point2D start, final SepStart s1, final SepStart s2, boolean add,
        ...
        Point2D st;
        Point2D diff;
        // This variable helps deal with switching isA. On the first rootNotFound switch it, but if
        // that doesn't fix it
        // just go ahead and fail.
        boolean justThrew = false;
        // This one lets us skip a point, if for whatever reason we can't find a connection at one
        // increment, skip over
        ...
        try {
            ...
            st = saddleConFinitePath(s1, s2, start.getX(), start.getY(), FinitePathType.SPIRAL,
                    null, transversal);
    ...
    private double minDist(final SepStart sep, final Point2D other, final double at,
        ...
        if (min == Double.MAX_VALUE) {
            ...
            double factor = 1D;
            System.out.println(
                    "Start: " + sep.getStart(factor * (xMax.get() - xMin.get()) / this.getWidth()));
    ...
    private Point2D saddleConLoop(SepStart s1, SepStart s2, Point2D center,
        ...
        for (int i = 0; i < 4; i++) {
            ...
            cds[i] = saddlePass(s1, s2, pts[i], transversal);
        }
        ...
        for (int i = 0; i < 4; i++) {
            if (cds[i] == -1 * cds[(i + 1) % 4]) {
                ...
                if (firstAdded) {
                    ...
                    /*
                     * t2 = new Thread(() -> { try { p2.set(saddleConMidpointPath(s1, s2,
                     * transversal, GeneratorFactory.getMidpointArcGenerator(finalPx /20, center,
                     * thetas[finalI + 1] + .1, thetas[finalI] - .1))); } catch (RootNotFound
                     * ignored) {} });
                     */
                    ...
                    /*
                     * t1 = new Thread(() -> { try { p1.set(saddleConMidpointPath(s1, s2,
                     * transversal, GeneratorFactory.getMidpointArcGenerator(finalPx/20, center,
                     * thetas[finalI + 1] + .1, thetas[finalI] - .1))); } catch (RootNotFound
                     * ignored) {} });
                     */
         ...
         * transversal); Point2D temp [] = new Point2D[2]; int currentVal = 0;
         * while(!gen.completed() && !Thread.interrupted()) { // try {
         ...
         * (RootNotFound r) { // gen.next(); } } if(currentVal < 1) throw new RootNotFound(); return
         * temp;
         */
    }
    ...
    Point2D saddleConMidpointPath(SepStart s1, SepStart s2, SaddleConTransversal transversal,
        ...
        if (!gen.done()) {
            System.out.println("defaulting out");
            // return saddleConFinitePath(s1, s2, prev1.getX(), prev1.getY(), FinitePathType.ARC,
            // prev2, transversal);
            throw new RootNotFound();
        } else
            return gen.getCurrentPoint();
    ...
    private Point2D saddleConnection(final SepStart s1init, final SepStart s2init, boolean isA,
        ...
        if (isA)
            ...
            tol = (in.yMax.get() - in.yMin.get()) / (1 * in.getHeight());
        ...
        while (dist1 > tol) {
            ...
            if (at < 2 * in.xMin.get() - in.xMax.get() || at > 2 * in.xMax.get() - in.xMin.get()
                    || bt < 2 * in.yMin.get() - in.yMax.get()
                    || bt > 2 * in.yMax.get() - in.yMin.get())
                throw new RootNotFound(true);
        }
        ...
        //
        // }
        // if(Math.abs(prev.minus(start).get(0, 0)) < tol && Math.abs(prev.minus(start).get(1,0)) <
        // tol)
        // {
    ...
    private double sepIntersect(SepStart s, double a, double b, Point2D[] ln, Point2D res)
            throws RootNotFound {
        double x, y;
