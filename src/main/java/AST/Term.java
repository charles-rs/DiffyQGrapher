...
public class Term extends Node {
    ...
    public Node differentiate(char c) {
        if (op == '*')
            return new Expression(new Term(a.clone(), b.differentiate(c), '*'),
                    new Term(a.differentiate(c), b.clone(), '*'), '+');
        else if (op == '/')
            return new Term(
                    new Expression(new Term(b.clone(), a.differentiate(c), '*'),
                            new Term(a.clone(), b.differentiate(c), '*'), '-'),
                    new Function(b.clone(), new Value(2.)), '/');
        else
    ...
    public Node collapse() {
        ...
        if (collapsedA instanceof Value && ((Value) collapsedA).type == ValueTypes.NUMBER
                && ((Value) collapsedA).val.equals(0.)
                || collapsedB instanceof Value && ((Value) collapsedB).type == ValueTypes.NUMBER
                        && ((Value) collapsedB).val.equals(0.) && this.op == '*') {
            return new Value(0.);
        } else if (collapsedA instanceof Value && ((Value) collapsedA).type == ValueTypes.NUMBER
                && ((Value) collapsedA).val.equals(1.) && this.op == '*') {
            return collapsedB;
        } else if (collapsedB instanceof Value && ((Value) collapsedB).type == ValueTypes.NUMBER
                && ((Value) collapsedB).val.equals(1.)) {
            return collapsedA;
        } else if (collapsedA instanceof Value && ((Value) collapsedA).type == ValueTypes.NUMBER
                && collapsedB instanceof Value && ((Value) collapsedB).type == ValueTypes.NUMBER) {
            if (op == '*')
    ...
    public boolean equals(Object other) {
        if (other instanceof Term) {
            return this.a.equals(((Term) other).a) && this.b.equals(((Term) other).b)
                    && this.op == ((Term) other).op;
        } else
            return false;
    }
}
